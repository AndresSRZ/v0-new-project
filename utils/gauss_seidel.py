...
def gauss_seidel(x0, A, b, error_type, Tol, niter):
    ...
    Re = f"Radio espectral de T = {max(np.abs(np.linalg.eigvals(T)))}"
    ...
    df = pd.DataFrame(xn, columns=[f"x{i+1}" for i in range(x1.size)])
    df.insert(0, "Iteration", N)
    df["E"] = E

    os.makedirs("app/tables", exist_ok=True)
    df.to_csv("app/tables/tabla_gaussSeidel.csv", index=False)

    plot_system(A, x1, b, "app/static/grafica_gaussSeidel.png")
    return r, N, xn, E, Re, c
